    ...
    , mScene(nullptr)
{
    ...
    int styleIdx
        = ui->ind_indicatorStyle->findText(Settings::inst()->value("chartRowIndicator").toString());
    ...
    connect(ui->guidelinesType, SIGNAL(currentIndexChanged(QString)),
            SLOT(chartUpdateGuidelines()));
    ...
    connect(ui->ind_fontComboBox, SIGNAL(currentFontChanged(const QFont&)),
            SLOT(indicatorUpdate()));
    ...
    connect(ui->ci_path, SIGNAL(textChanged(const QString&)),
            SLOT(chartImageUpdatePath(const QString&)));
...
void
PropertiesDock::loadProperties(Guidelines guidelines)
{
...
void
PropertiesDock::tabChanged(int tabNumber)
{
    if (tabNumber == -1)
    {
        clearUi();
...
void
PropertiesDock::setupStitchCombo()
{
...
bool
PropertiesDock::updateGuidelines()
{
    bool changed = false;
...
PropertiesData
PropertiesDock::selectionProperties()
{
    PropertiesData props;
    bool firstPass = true;

    bool angleMixed = false, xScaleMixed = false, yScaleMixed = false, xPositionMixed = false,
         yPositionMixed = false, stitchMixed = false, colorMixed = false, bgColorMixed = false;
...
void
PropertiesDock::showUi(PropertiesDock::UiSelection selection, int count)
{
    Q_UNUSED(count);
...
void
PropertiesDock::showSingleChartImage()
{
...
void
PropertiesDock::showMultiCell()
{
    // qDebug() << "multi cell";
}

void
PropertiesDock::showSingleIndicator()
{
    PropertiesData p = selectionProperties();
...
void
PropertiesDock::showMultiIndicator()
{
}

void
PropertiesDock::showMixedObjects()
{
    // qDebug() << "mixed objs";
    ui->itemGroup->show();
}
...
void
PropertiesDock::updateGuidelinesUi()
{
    ...
    else
    {
        ...
        else if (ui->guidelinesType->currentText() == "Rounds")
        {  // rounds
        ...
        else if (ui->guidelinesType->currentText() == "Triangles")
        {  // rounds
...
void
PropertiesDock::indicatorUpdate()
{
...
void
PropertiesDock::chartUpdateAlignAngle(bool state)
{
    emit propertiesUpdated("AlignAngle", QVariant(state));
}

void
PropertiesDock::chartUpdateChartCenter(bool state)
{
    emit propertiesUpdated("ChartCenter", QVariant(state));
}

void
PropertiesDock::chartUpdateGuidelines()
{
    if (updateGuidelines())
    {
        QVariant value;
...
void
PropertiesDock::propertyUpdated()
{
    updateDialogUi();
}
...
void
PropertiesDock::cellUpdateAngle(double angle)
{
    emit propertiesUpdated("Angle", QVariant(angle));
}

void
PropertiesDock::cellUpdatePositionX(double positionX)
{
    emit propertiesUpdated("PositionX", QVariant(positionX));
}

void
PropertiesDock::cellUpdatePositionY(double positionY)
{
    emit propertiesUpdated("PositionY", QVariant(positionY));
}

void
PropertiesDock::cellUpdateScaleX(double scale)
{
    emit propertiesUpdated("ScaleX", QVariant(scale));
}

void
PropertiesDock::itemGroupUpdateScale(double scale)
{
    emit propertiesUpdated("ScaleX", QVariant(scale));
...
void
PropertiesDock::cellUpdateScaleY(double scale)
{
    emit propertiesUpdated("ScaleY", QVariant(scale));
}

void
PropertiesDock::cellUpdateStitch(QString stitch)
{
    emit propertiesUpdated("Stitch", QVariant(stitch));
}

void
PropertiesDock::cellDeleteItems()
{
    emit propertiesUpdated("Delete", QVariant(true));
}

void
PropertiesDock::cellUpdateFgColor()
{
    QColorDialog cdlg;
...
void
PropertiesDock::cellUpdateBgColor()
{
    QColorDialog cdlg;
    QColor color = cdlg.getColor();
    QVariant v = color;
...
void
PropertiesDock::chartImageUpdatePath(const QString& path)
{
    emit propertiesUpdated("ChartImagePath", QVariant(path));
}

void
PropertiesDock::chartImageChoosePath()
{
...
void
PropertiesDock::chartImageUpdateZLayer(const QString& layer)
{
