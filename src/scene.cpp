...
QDataStream&
operator<<(QDataStream& stream, Guidelines& guidelines)
{
...
QDebug
operator<<(QDebug d, Guidelines& guidelines)
{
...
bool
Guidelines::operator==(const Guidelines& other) const
{
    bool isEqual = true;
...
QDebug
operator<<(QDebug d, IndicatorProperties& properties)
{
...
Scene::Scene(QObject* parent)
    ...
    , mCellStartPos(0, 0)
    , mLeftButtonDownPos(0, 0)
    , mDiff(0, 0)
    , mSelectMode(BoxSelect)
    , mRubberBandStart(0, 0)
    , mMode(Scene::StitchEdit)
    , mEditStitch("ch")
    , mEditFgColor(Qt::black)
    , mEditBgColor(Qt::white)
    , mOldScale(1.0, 1.0)
    , mOldTransform()
    , mOldSize(1, 1)
    , mOrigin(0, 0)
    , mDefaultSize(32.0, 96.0)
    , mDefaultStitch("ch")
{
...
{
    QStringList modes;
    modes << tr("Stitch Edit") << tr("Color Edit") << tr("Row Edit") << tr("Rotation Edit")
          << tr("Scale Edit") << tr("Indicator Edit");
    return modes;
}
...
QGraphicsItem*
Scene::selectableItemAt(const QPointF& pos)
{
...
Scene::addItem(QGraphicsItem* item)
{
    ...
    case Cell::Type:
    {
        ...
        connect(c, SIGNAL(stitchChanged(QString, QString)),
                SIGNAL(stitchChanged(QString, QString)));
    ...
    case Indicator::Type:
    {
        ...
        // connect(i, SIGNAL(selectedChange(QGraphicsItem*)), this,
...
void
Scene::updateRubberBand(int dx, int dy)
{
...
void
Scene::contextMenuEvent(QGraphicsSceneContextMenuEvent* e)
{
    QMenu menu;
...
void
Scene::keyReleaseEvent(QKeyEvent* keyEvent)
{
...
void
Scene::stitchModeKeyRelease(QKeyEvent* keyEvent)
{
...
void
Scene::angleModeKeyRelease(QKeyEvent* keyEvent)
{
...
void
Scene::scaleModeKeyRelease(QKeyEvent* keyEvent)
{
    // TODO transform_refactor
...
void
Scene::mousePressEvent(QGraphicsSceneMouseEvent* e)
{
    if (selectedItems().count() > 0)
        mHasSelection = true;
    ...
    if (e->buttons() & Qt::LeftButton
        && (e->modifiers() != Qt::ShiftModifier || selectedItems().count() >= 1))
        QGraphicsScene::mousePressEvent(e);

    // FIXME: there has to be a better way to keep the current selection.
    // FIXME 2: I commented out the following two lines of codes, which fixed ctrl-selection. No
    // idea why. Hope I didn't break anything else. if(mHasSelection && e->modifiers() ==
    // Qt::ControlModifier)
    ...
    if (mCurItem)
    {
        ...
        case Cell::Type:
        {
            ...
            mDiff = QSizeF(e->scenePos().x() - mCellStartPos.x(),
                           e->scenePos().y() - mCellStartPos.y());
            break;
        }
        case Indicator::Type:
        {
...
void
Scene::mouseMoveEvent(QGraphicsSceneMouseEvent* e)
{
    switch (mMode)
    {
    case Scene::StitchEdit:
...
void
Scene::mouseReleaseEvent(QGraphicsSceneMouseEvent* e)
{
...
void
Scene::snapGraphicsItemToGrid(QGraphicsItem& item)
{
...
QPointF
Scene::snapPositionToGrid(const QPointF& pos) const
{
...
QPointF
Scene::snapPositionToTriangles(const QPointF& pos) const
{
...
QPointF
Scene::snapPositionToRows(const QPointF& pos) const
{
    // get the needed variables
    QPointF center(0, 0);
    if (mCenterSymbol)
...
QPointF
Scene::snapPositionToRounds(const QPointF& pos) const
{
    // get the needed variables
    QPointF center(0, 0);
    if (mCenterSymbol)
    ...
    return QPointF(cos(relPosSnappedAngle) * relPosSnappedDistance,
                   sin(relPosSnappedAngle) * relPosSnappedDistance)
           + center;
}

void
Scene::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* e)
{
...
void
Scene::colorModeMouseMove(QGraphicsSceneMouseEvent* e)
{
    if (e->buttons() != Qt::LeftButton)
        return;
...
void
Scene::colorModeMouseRelease(QGraphicsSceneMouseEvent* e)
{
    Q_UNUSED(e);
    if (!mCurItem || mCurItem->type() != Cell::Type)
        return;
...
void
Scene::indicatorModeMouseMove(QGraphicsSceneMouseEvent* e)
{
    if (e->buttons() != Qt::LeftButton)
        return;
...
void
Scene::indicatorModeMouseRelease(QGraphicsSceneMouseEvent* e)
{
    if (e->isAccepted())
        return;
...
qreal
Scene::scenePosToAngle(QPointF pt)
{
...
void
Scene::angleModeMousePress(QGraphicsSceneMouseEvent* e)
{
    Q_UNUSED(e);
    if (!mCurItem)
        return;
    ...
    {
        localPivot
            = QPointF(mCurItem->boundingRect().center().x(), mCurItem->boundingRect().bottom());
    }
...
void
Scene::angleModeMouseMove(QGraphicsSceneMouseEvent* e)
{
    if (!mCurItem)
        return;
...
void
Scene::angleModeMouseRelease(QGraphicsSceneMouseEvent* e)
{
    Q_UNUSED(e);
    if (!mCurItem)
        return;
...
void
Scene::scaleModeMousePress(QGraphicsSceneMouseEvent* e)
{
    Q_UNUSED(e);
    if (!mCurItem)
        return;
...
void
Scene::scaleModeMouseMove(QGraphicsSceneMouseEvent* e)
{
    if (!mCurItem)
        return;
    ...
    QSizeF currentSize
        = QSizeF(originalSize.width() * mOldScale.x(), originalSize.height() * mOldScale.y());

    QSizeF newSize = QSizeF(currentSize.width() + delta.x(), currentSize.height() + delta.y());

    QPointF neededScale
        = QPointF(newSize.width() / originalSize.width(), newSize.height() / originalSize.height());
...
void
Scene::scaleModeMouseRelease(QGraphicsSceneMouseEvent* e)
{
    Q_UNUSED(e);
    if (mMoving)
        return;
...
void
Scene::rowEditMousePress(QGraphicsSceneMouseEvent* e)
{
    if (!e->buttons() == Qt::LeftButton)
        return;
...
void
Scene::rowEditMouseMove(QGraphicsSceneMouseEvent* e)
{
    if (!e->buttons() == Qt::LeftButton)
        return;
...
void
Scene::rowEditMouseRelease(QGraphicsSceneMouseEvent* e)
{
    Q_UNUSED(e);
...
void
Scene::stitchModeMousePress(QGraphicsSceneMouseEvent* e)
{
    Q_UNUSED(e);
}

void
Scene::stitchModeMouseMove(QGraphicsSceneMouseEvent* e)
{
    Q_UNUSED(e);
    if (!mCurItem)
        return;
...
void
Scene::stitchModeMouseRelease(QGraphicsSceneMouseEvent* e)
{
    if (e->isAccepted())
        return;
...
void
Scene::setBackground(bool enabled)
{
    mbackgroundIsEnabled = enabled;
}

void
Scene::drawRowLines(int row)
{
    if (grid.count() <= row)
        return;
...
void
Scene::alignSelection(int alignmentStyle)
{
    if (selectedItems().count() <= 0)
        return;
...
void
Scene::distributeSelection(int distributionStyle)
{
    if (selectedItems().count() <= 0)
        return;
...
void
Scene::align(int vertical, int horizontal)
{
    ...
    foreach (QGraphicsItem* i, selectedItems())
    {
        ...
        {
            newX = i->pos().x()
                   + ((newX - i->sceneBoundingRect().x()) - i->sceneBoundingRect().width() / 2);
        }
        else if (horizontal == 3)
        {
            newX = i->pos().x()
                   + ((newX - i->sceneBoundingRect().x()) - i->sceneBoundingRect().width());
        }
        ...
        {
            newY = i->pos().y()
                   + ((newY - i->sceneBoundingRect().y()) - i->sceneBoundingRect().height() / 2);
        }
        else if (vertical == 3)
        {
            newY = i->pos().y()
                   + ((newY - i->sceneBoundingRect().y()) - i->sceneBoundingRect().height());
        }
...
Scene::generateGuidelinesRows(int spacingW, int spacingH, int columns, int rows, QPointF center)
{
    ...
    {
        i = addLine(c * spacingW + center.x(), center.y(), c * spacingW + center.x(),
    ...
    {
        i = addLine(center.x(), center.y() + r * spacingH, center.x() + spacingW * columns,
                    center.y() + r * spacingH);
    ...
    int /*spacingW*/, int spacingH, int columns, int rows, QPointF center)
{
    ...
    for (int r = 0; r <= rows; r++)
    {
        Guideline* g = new Guideline(
            QRectF(center.x() + -(r + 1) * spacingH, center.y() + -(r + 1) * spacingH,
                   2 * (r + 1) * spacingH, 2 * (r + 1) * spacingH),
            0, this);
        g->setZValue(-1);
...
void
Scene::generateGuidelinesTriangles(
    int spacingW, int spacingH, int columns, int rows, QPointF center)
{
    Q_UNUSED(columns);
    QGraphicsItem* i = nullptr;
    ...
    for (int r = 0; r < rows; r++)
    {
        ...
        i = addLine(-offsetX + center.x(), height + center.y(), offsetX + center.x(),
                    height + center.y());
        i->setZValue(-1);
        mGuidelinesLines.append(i);
    }
    ...
    for (int c = 0; c < rows; c++)
    {
        ...
        i->setZValue(-1);
        mGuidelinesLines.append(i);
        i = addLine(-startX + center.x(), startY + center.y(), -endX + center.x(),
                    endY + center.y());
        i->setZValue(-1);
        mGuidelinesLines.append(i);
    }
...
QList<QGraphicsItem*>
Scene::sortItemsHorizontally(QList<QGraphicsItem*> unsortedItems, int sortEdge)
{
    ...
    foreach (QGraphicsItem* item, unsortedItems)
    {
        ...
        {  // sortEdge == 1 or any other un-acceptable value.
...
QList<QGraphicsItem*>
Scene::sortItemsVertically(QList<QGraphicsItem*> unsortedItems, int sortEdge)
{
    QList<QGraphicsItem*> sorted;
...
QRectF
Scene::selectionBoundingRect(QList<QGraphicsItem*> items, int vertical, int horizontal)
{
    qreal left = sceneRect().right();
    qreal right = sceneRect().left();
    qreal top = sceneRect().bottom();
    ...
    QRectF topLeft(QPointF(left, top), QPointF(right, bottom)),
        ...
        bottomRight(QPointF(left, top), QPointF(right, bottom)), final;
...
void
Scene::distribute(int vertical, int horizontal)
{
    ...
    */
    ...
    foreach (QGraphicsItem* i, unsorted)
    {
        ...
        {
            newX = i->pos().x()
                   + ((selectionRect.left() + (idxX * spaceH)) - offsetX
                      - i->sceneBoundingRect().x());
        }
        ...
        {
            newY = i->pos().y()
                   + ((selectionRect.top() + (idxY * spaceV)) - offsetY
                      - i->sceneBoundingRect().y());
        }
...
void
Scene::distributeToPath()
{
}

void
Scene::resizeScene(QRectF size)
{
    setSceneRect(size);
}

void
Scene::createRowsChart(int rows, int cols, QString defStitch, QSizeF rowSize)
{
...
void
Scene::createBlankChart()
{
}

void
Scene::arrangeGrid(QSize grd, QSize alignment, QSize spacing, bool useSelection)
{
...
void
Scene::gridAddRow(QList<Cell*> row, bool append, int before)
{
    if (append)
    {
        grid.append(row);
...
void
Scene::propertiesUpdate(QString property, QVariant newValue)
{
    ...
    {  // all options that require operations on a per stitch level.
        ...
        foreach (QGraphicsItem* i, selectedItems())
        {
            ...
            if (property == "Angle")
            {
                ...
                undoStack()->push(
                    new SetItemRotation(i, rotation, ChartItemTools::getRotationPivot(i)));
            }
            ...
            else if (property == "ScaleX")
            {
                ...
                undoStack()->push(new SetItemScale(i, scale, ChartItemTools::getScalePivot(i)));
            }
...
void
Scene::updateDefaultStitchColor(QColor originalColor, QColor newColor)
{
    foreach (QGraphicsItem* item, items())
    {
...
QGraphicsItem*
Scene::copy_rec(QGraphicsItem* item, QPointF displacement)
{
    if (item->type() == Cell::Type)
    {
        // get the cell
    ...
    else if (item->type() == ItemGroup::Type)
    {
        ...
        mPivotPt = QPointF(g->boundingRect().center().x(), g->boundingRect().bottom());
...
void
Scene::showProperties()
{
    emit showPropertiesSignal();
}

void
Scene::copy(int direction)
{
    if (selectedItems().count() <= 0)
        return;
...
void
Scene::copyRecursively(QDataStream& stream, QList<QGraphicsItem*> items)
{
    foreach (QGraphicsItem* item, items)
    {
        stream << item->type();
        ...
        {
            Cell* c = qgraphicsitem_cast<Cell*>(item);
            stream << c->name() << c->color() << c->bgColor() << ChartItemTools::getRotation(c)
                   << ChartItemTools::getRotationPivot(c) << ChartItemTools::getScaleX(c)
                   << ChartItemTools::getScaleY(c) << ChartItemTools::getScalePivot(c)
                   << c->transformOriginPoint() << c->pos();
            break;
        }
        case Indicator::Type:
        {
            Indicator* i = qgraphicsitem_cast<Indicator*>(item);
            stream << i->scenePos() << i->text() << ChartItemTools::getRotation(i)
                   << ChartItemTools::getRotationPivot(i) << ChartItemTools::getScaleX(i)
                   << ChartItemTools::getScaleY(i) << ChartItemTools::getScalePivot(i);
            break;
        }
        case ItemGroup::Type:
        {
            ...
            stream << group->pos() << group->childItems().count()
        ...
        case ChartImage::Type:
        {
            ...
            stream << image->filename() << image->pos() << ChartItemTools::getRotation(image)
                   << ChartItemTools::getRotationPivot(image) << ChartItemTools::getScaleX(image)
                   << ChartItemTools::getScaleY(image) << ChartItemTools::getScalePivot(image);
            break;
        }
        case Guideline::Type:
            qDebug() << "guideline";
...
Scene::paste()
{
    ...
    if (Settings::inst()->value("pasteOffset").toString().compare(tr("On mouse cursor")) == 0)
    {
        if (items.count() > 0)
        {
            ...
            QPointF mousePos
                = views().first()->mapToScene(views().first()->mapFromGlobal(QCursor::pos()));
...
void
Scene::insertImage(const QString& filename, QPointF pos)
{
...
void
Scene::pasteRecursively(QDataStream& stream, QList<QGraphicsItem*>* group)
{
    QPointF offSet;
...
QRectF
Scene::selectedItemsBoundingRect(QList<QGraphicsItem*> items)
{
    if (items.count() <= 0)
        return QRectF();
...
ItemGroup*
Scene::group(QList<QGraphicsItem*> items, ItemGroup* g)
{
...
void
Scene::addToGroup(int groupNumber, QGraphicsItem* i)
{
...
ItemGroup*
Scene::getGroup(int groupNumber)
{
    return mGroups[groupNumber];
}

void
Scene::editorLostFocus(Indicator* item)
{
...
void
Scene::editorGotFocus(Indicator* item)
{
 ...
 */
void
Scene::addLayer(const QString& name)
{
...
void
Scene::addLayer(const QString& name, unsigned int uid)
{
...
void
Scene::addLayer(ChartLayer* layer)
{
    mLayers[layer->uid()] = layer;
...
void
Scene::addLayerUndoable(const QString& name)
{
    mUndoStack.push(new AddLayer(this, new ChartLayer(name)));
}

void
Scene::addLayerUndoable(const QString& name, unsigned int uid)
{
    mUndoStack.push(new AddLayer(this, new ChartLayer(uid, name)));
}

void
Scene::removeLayer(unsigned int uid)
{
...
void
Scene::removeLayerUndoable(unsigned int uid)
{
...
void
Scene::selectLayer(unsigned int uid)
{
    clearSelection();
...
void
Scene::setSnapAngle(bool state)
{
    mSnapAngle = state;
}

void
Scene::setShowChartCenter(bool state)
{
    ...
    if (mShowChartCenter)
    {
        if (!mCenterSymbol)
        {
            ...
            mCenterSymbol = addEllipse(rect.center().x() - radius, rect.center().y() - radius,
...
void
Scene::createRoundsChart(int rows, int cols, QString stitch, QSizeF rowSize, int increaseBy)
{
    mDefaultSize = rowSize;
...
void
Scene::setCellPosition(int row, int column, Cell* c, int columns)
{
...
void
Scene::setEditMode(EditMode mode)
{
    mMode = mode;
...
void
Scene::setSelectMode(Scene::SelectMode mode)
{
    mSelectMode = mode;
}

Scene::SelectMode
Scene::selectMode() const
{
    return mSelectMode;
}

void
Scene::highlightIndicators(bool state)
{
    foreach (Indicator* i, mIndicators)
    {
        if (!state)
...
void
Scene::setGuidelinesType(QString guides)
{
...
void
Scene::replaceStitches(QString original, QString replacement)
{
...
void
Scene::replaceColor(QColor original, QColor replacement, int selection)
{
